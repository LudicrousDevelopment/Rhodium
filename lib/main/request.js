...
function Request(ctx) {
  return function (request, response) {
    ...
    var url = ctx.encoding
      .decode(request.url.replace(ctx.prefix, ""))
      .replace("https://", "https:/")
      .replace("https:/", "https://") /*.replace('.map', '')*/
      .replace(/-$/g, "");
    ...
    try {
      ...
      return response.writeHead(503).end(e.toString());
    }
    var Url = new URL(url);
    try {
      try {
        eval(Url.protocol.replace(":", ""));
      } catch {
    ...
    const httpRequest = eval(Url.protocol.replace(":", ""))
      .request(
        Url,
        { method: request.method, headers: headers, rawHeaders: [] },
        (res) => {
          var chunks = [];
          res
            .on("data", (e) => chunks.push(e))
            .on("end", () => {
              //if (request.headers['x-replit-user-id']) url = url.replace('https://', 'https:/')
              var textData = ctx.decompress(res, chunks);
              try {
                var type = (res.headers["content-type"] || "text/plain").split(
                  ";"
                )[0];
                if (type == "text/html")
                  textData = ctx.rewrite.HTML(
                    textData.toString(),
                    request,
                    ctx
                  );
                ...
                if (
                  ...
                  textData = ctx.rewrite.JS(textData.toString(), { Url: Url });
                if (type == "text/css")
              ...
              response
                .writeHead(res.statusCode, ctx.header.response(request, res))
                .end(textData);
            });
        }
      )
    ...
    if (!response.writableEnded) {
      request
        .on("data", (data) => httpRequest.write(data))
        .on("end", () => httpRequest.end());
    } else {
      httpRequest.end();
    }
  };
