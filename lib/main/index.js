...
Object.defineProperty($Rhodium, "hlocation", {
  ...
  set(val) {
    location.href = $Rhodium.url.encode(val, {
      Url: new URL($Rhodium.location.href)
    });
  }
});
    ...
    .map((e) => {
      return e
        ...
        .map((a) => {
          if (
            a.startsWith("http") ||
            (a.startsWith("/") && !a.startsWith($RConfig.prefix))
          ) {
            var url = $Rhodium.url.encode(a, {
              Url: new URL($Rhodium.location.href)
            });
          }
  ...
  return {
      ...
      try {
        ...
        url =
          typeof window == "object"
            ? url.replace(location.hostname, $Rhodium.location.hostname)
            : url;
    ...
    decode(url) {
      return url
        ? $Rhodium.encoding
            .decode(
              url.replace($Rhodium.prefix, "").replace(location.origin, "")
            )
            .replace("https://", "https:/")
            .replace("https:/", "https://")
        : undefined;
...
$Rhodium.Location = function (url, window = _window) {
  ...
  var go = function (v) {
    return window.$Rhodium.url.encode(v, { Url: new URL($RConfig.url) });
  };
  ...
  [
    ...
    Object.defineProperty(window.$Rhodium.location, prop, {
      ...
      set(val) {
        return (window.location[prop] = window.$Rhodium.go(
          window.$Rhodium.Url.href.replace(window.$Rhodium.Url[prop], val)
        ));
      }
    });
  });
  ["assign", "replace", "toString", "reload"].forEach((prop) => {
    Object.defineProperty(window.$Rhodium.location, prop, {
      get() {
        return new Function(
          "arg",
          `return window.location.${prop}(arg?${prop !== "reload" && prop !== "toString" ? "$Rhodium.go(arg)" : "arg"}:null)`
        );
      },
      set(val) {
        return val;
...
$Rhodium.css = function (body) {
  function CSSRewriter(ctx) {
    return function CSS(data, context) {
      const cont = context;
      cont.Url = new URL(
        ctx.encoding.decode($RConfig.url.replace(ctx.prefix, ""))
      );
      return data
        .replace(/url\("(.*?)"\)/gi, (str) => {
...
Object.defineProperty(window.document, "cookie", {
    ...
    cookieString
      ...
      .map((e) =>
        e
          .split("=")
          .map((a) =>
            a.startsWith($Rhodium.location.hostname)
              ? cookies.push(
                  `${e.split("=")[0].replace($Rhodium.location.host, "")}=${e.split("=")[1]}`
                )
              : null
          )
      );
  ...
  set(val) {
    var newVal = val
      .split("=")
      .map((e, i) =>
        val.split("=").indexOf(e) == 0 ? $Rhodium.location.host + e : e
      )
      .join("=");
        ...
        if (_config.action[0] == "srcset") {
          _config.tags.forEach((tag) => {
            document.querySelectorAll(`*[${tag}]`).forEach((node) => {
              ...
              node.setAttribute(
                tag,
                $Rhodium.RewriteSrcset(node.getAttribute(tag))
              );
            });
          });
        }
  ...
  element.prototype.setAttribute = new Proxy(element.prototype.setAttribute, {
    apply(target, thisArg, [element_attribute, value]) {
      ...
      if (
        ...
        value = $Rhodium.url.encode(value, { Url: new URL($RConfig.url) });
      return Reflect.apply(target, thisArg, [element_attribute, value]);
    }
  });
  [
                        ...
                        if (!val.$Rhodium) {
                          ...
                          if (that.src.includes("hcaptcha"))
                            val["id"] = new URLSearchParams(
                              that.contentWindow.$Rhodium.location.search
                            ).get("id");
                        }
                      } catch (e) {}
                      break;
                ...
                apply(target, that, [val]) {
                  ...
                  switch (config.action) {
                    case "url":
                      ...
                      newVal = $Rhodium.url.encode(newVal, {
                        Url: new URL($RConfig.url)
                      });
                      break;
                    case "srcset":
...
window.Worker = new Proxy(window.Worker, {
  construct(t, a) {
    a[0] = $Rhodium.url
      .encode(a[0], { Url: new URL($RConfig.url) })
      .replace("https://", "https:/");
    console.log(a[0]);
...
history.pushState(
  {},
  "",
  $RConfig.prefix + $Rhodium.encoding.encode($Rhodium.location.href)
);
...
window.fetch = new Proxy(window.fetch, {
  apply(t, g, a) {
    ...
    if (typeof a[0] == "object") {
      a[0] == {};
      a[0].url = $Rhodium.go(arg.url);
      Object.assign(a[0], arg);
      return Reflect.apply(t, g, a);
    } //.url = $Rhodium.url.encode(a[0].url, {Url: new URL($RConfig.url)});return Reflect.apply(t, g, a)}
    if (a[0]) a[0] = $Rhodium.url.encode(a[0], { Url: new URL($RConfig.url) });
    return Reflect.apply(t, g, a);
  }
});

$Rhodium.urlRequest = Object.getOwnPropertyDescriptor(
...
window.WebSocket = new Proxy(window.WebSocket, {
  construct(t, a) {
    ...
    a[0] =
      ...
      $Rhodium.prefix +
      "?ws=" +
      $Rhodium.encoding.encode(a[0] + origin);
    return Reflect.construct(t, a);
  }
});
        ...
        if (name == "setItem")
          return function () {
            return LocalStorage.setItem(
              $Rhodium.location.hostname + arguments[0],
              arguments[1]
            );
          };
        if (name == "removeItem")
